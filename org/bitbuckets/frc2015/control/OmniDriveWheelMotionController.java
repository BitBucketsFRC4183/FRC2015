...
public class OmniDriveWheelMotionController implements MotionController {
    ...
    /**
     * Gets the speed of a wheel for omni-directional drive with wheels in any configuration. <code>vwp</code> is the velocity vector for the wheel without accounting for the fact that it can't rotate.
     *
    ...
    public double getWheelSpeed(double xc, double yc, double xw, double yw, double theta, double vxc, double vyc, double rotc) {
        ...
        double thetaR = Math.atan((yw - yc) / (xw - xc));
